...
for iter = 1 : maxgen
    for i = 1 : FoodNum
        ...
        nextfood(changenode) = sourcefood(changenode) + (2*rand - 1) * (neighbourfood(changenode) - sourcefood(changenode));
    ...
    for k = 1 : FoodNum
        ...
        neighbourfood = X(foodindex,:);
        changenode = randi([1 d]);
        nextfood(changenode) = nextfood(changenode) + (2*rand - 1) * (neighbourfood(changenode) - sourcefood(changenode));
        if nextfood(changenode) < lb(changenode)
            nextfood(changenode) = lb(changenode);
        end
        if nextfood(changenode) > ub(changenode)
